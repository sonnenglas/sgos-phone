...
Part of [SGOS](https://sgos-infra.sgl.as) infrastructure. Deployed on Hornbill server.

### Quick Start (Local)

```bash
git clone https://github.com/sonnenglas/sgos-phone.git
cd sgos-phone

# Decrypt secrets (requires age key in ~/.config/sops/age/keys.txt)
...
**Server:** Hornbill (`/srv/apps/sgos-phone/`)
**URL:** https://phone.sgl.as
...
Auto-deployment is configured via GitHub webhook → Toucan → SSH to Hornbill.

### Database

PostgreSQL runs in a separate container. Data persists in `./pgdata` directory.
...
- `.env.sops` - Encrypted secrets (committed to git)
- `.env` - Decrypted secrets (gitignored, generated on deploy)

See [SGOS Secrets Documentation](https://sgos-infra.sgl.as/operations/secrets) for details.
...
All secrets are stored in `.env.sops` (encrypted with SOPS/age). See `app.json` for full list.
...
- **Email**: Postmark
- **Secrets**: SOPS + age (encrypted .env.sops)
